...
function listadoProductoPedidoSucursal($categoria,$campo,$frase,$pag,$TotalReg,$moneda,$idsucursal){
	Global $objProducto;
	Global $EncabezadoTabla;
	Global $tipocambio;
	$regxpag=10;
...
function genera_cboUnidadPedidoSucursal($idproducto,$moneda,$idsucursal){
	Global $tipocambio;
	require("../negocio/cls_listaunidad.php");
	$ObjListaUnidad = new clsListaUnidad();
	$consulta = $ObjListaUnidad->buscarparaventaconxajax($idsucursal,$idproducto,$moneda,$tipocambio);

	$Unidads="<table><tr><td><select name='cboUnidad' id='cboUnidad' onchange='cambiaStockPedidoSucursal()'>";
	while($registro=$consulta->fetchObject()){
		if($registro->idunidad==$registro->idunidadbase){ 
			$seleccionar="Selected";
			$precioventa=$registro->precioventa;
...
function cambiaStockPedidoSucursal($idproducto,$idunidad,$moneda,$idsucursal){
	Global $tipocambio;
	require("../negocio/cls_stockproducto.php");
	$ObjStockProducto = new clsStockProducto();
	$consulta = $ObjStockProducto->obtenerStock($idsucursal,$idproducto,$idunidad);
...
function listadopersona($campo,$frase,$TotalReg,$origen){
    ...
    $cadena="&nbsp;Registros encontrados: $nr de $nr1";
    $registros="
	<table class=registros width='410'>
    <tr>";
	for($i=0;$i<count($EncabezadoTabla);$i++){
	$registros.="<th>".$EncabezadoTabla[$i]."</th>";
	}
...
function generarventas($numero,$idtipodoc,$fechainicio,$fechafin,$formapago, $conguia){
	...
	$rst = $objMovimiento->consultarPedido(5,NULL,$idsucursal,$numero,$idtipodoc,$fechainicio,$fechafin,$formapago, $conguia);
	...
	while($dato = $rst->fetchObject()){
	   ...
	   if($cont%2) $estilo="par"; else $estilo="impar";
		if($dato->estado=='A'){
		$estado='Anulado';
		$registro.="<tr  class='$estilo' style='color:#FF0000'>";//<td>".$dato->documento."</td>";
		}else{
...
/*	$registro.="<td align='center'>".$dato->subtotal."</td>";
	...
	$registro.="<td width='54'><a href='frm_detallepedido.php?IdVenta=".$dato->idmovimiento."'>Ver Detalle </a></td>";
...
function generarpedidoventas($numero,$idtipodoc,$fechainicio,$fechafin,$formapago, $conguia){
	Global $idsucursal;
	Global $idusuario;
	if($idtipodoc==0){$idtipodoc=NULL;}
	if($formapago=='F'){$formapago=NULL;}
...
/*	$registro.="<td align='center'>".$dato->subtotal."</td>";
	...
	$registro.="<td align='center'>".$estado."</td>";
	$registro.="<td width='54'><a href='frm_detallepedido.php?origen=PA&IdVenta=".$dato->idmovimiento."'>Ver Detalle </a></td>";
...
*/	/*if($dato->idtipodocumento==1){
	...
	if($dato->numguia!=2){
		$registro.="<td width='54'><a href='frm_venta.php?accion=ACTUALIZAR&IdPedido=".$dato->idmovimiento."'>Generar Venta</a></td>";
	}else{
		$registro.="<td>-</td>";
	}
...
function referenciarventas($numero,$idtipodoc,$fechainicio,$formapago){
	...
	//$rst = $objMovimiento->consultarPedido(2,NULL,NULL,$numero,$idtipodoc,$fechainicio,NULL,$formapago);
	
	//si consultamo solo anuladas
		$rst = $objMovimiento->consultarPedidoanulada(2,$numero, $idtipodoc,$fechainicio, $formapago);
	while($dato = $rst->fetchObject()){
		if($dato->numero=="0"){
		$numero='Sin Ent.';
		}else{
...
if($origen=='100'){
$registro.="<fieldset><legend><strong>B&Uacute;SQUEDA TRANSPORTISTAS:</strong></legend>
...
elseif($origen=='0' || $origen=='3'){
$registro.="<fieldset><legend><strong>B&Uacute;SQUEDA PERSONAS:</strong></legend>
...
$registro.="<fieldset><legend><strong>B&Uacute;SQUEDA SUCURSAL:</strong></legend>
<input type='hidden' name='pagPersona' id='pagPersona' value='1'>
<input type='hidden' name='TotalRegPersona' id='TotalRegPersona'>
<input type='hidden' name = 'txtOrigenPersona' id='txtOrigenPersona' value = '".$origen."'>
  <div id='divbusquedaPersona' class='textoazul'>   
    <div align='left' class='alignleft' style='display:none;'>&nbsp;Por:
  <select name='campoPersona' id='campoPersona' onChange='buscarPersona()'>
    <option value=\"CONCAT(apellidos,'',nombres)\">Apellidos y Nombres</option>
	<option value='NroDoc'>Nro Doc.</option>
  </select></div>
  <div align='left' class='alignleft'><br>
      &nbsp;Descripción:
  <input type='text' name='frasePersona' id='frasePersona' onKeyUp='buscarPersona()'>
  <button type='button' class='boton' onClick=\"window.open('mant_persona.php?accion=NUEVO&origen=DOC','_blank','width=380,height=480');\">NUEVO</button>
    </div>
...
function generarventasinicio(){
	...
	$rst = $objMovimiento->consultarPedido(5,NULL,$idsucursal,NULL,NULL,date("Y-".$m."-d"),$fechaproceso,NULL,NULL);
...
function generarpedidosventasinicio(){
	Global $idsucursal;
	Global $idusuario;
	Global $fechaproceso;
	$registro.="<table width='1031'  class='registros'>
...
$registro.="<input  id='txtNroDoc' name='txtNroDoc' type='text' size='15' maxlength='15' value='";
	...
	$registro.=$objMov->consultar_numero_sigue(5,$documento,$idsucursal); 
	$registro.='-'.$year;
	$registro.="' >";
	$registro=utf8_encode($registro);
	$obj=new xajaxResponse();
...
if($num_mov==0){
	...
	$ObjMov->insertar($_SESSION['IdSucursal'],3,1,$txtNroDoc,10,'A',$_SESSION['FechaProceso'],'S',$montoAperturaSoles/1.19,($montoAperturaSoles/1.19)*(19/100),$montoAperturaSoles,$_SESSION['IdUsuario'],$_SESSION['IdSucursal'],$_SESSION['IdUsuario'],'','',"Caja Aperturada en Soles el ".date('Y-m-d'),'N');
		...
		$ObjMov->insertar($_SESSION['IdSucursal'],3,1,$numsiguiente,10,'A',$_SESSION['FechaProceso'],'D',$montoAperturaDolares/1.19,($montoAperturaDolares/1.19)*(19/100),$montoAperturaDolares,$_SESSION['IdUsuario'],$_SESSION['IdSucursal'],$_SESSION['IdUsuario'],'','',"Caja Aperturada en Dolares el ".date('Y-m-d'),'N');
	...
	if($cierre==0){
		...
		$ObjMov->insertar($_SESSION['IdSucursal'],3,2,$numerodoc,11,'A',$fechacierre,'S',$montoAperturaSoles/1.19,($montoAperturaSoles/1.19)*(19/100),$montoAperturaSoles,$_SESSION['IdUsuario'],$_SESSION['IdSucursal'],$_SESSION['IdUsuario'],'','',"Cierre de Caja en Soles del".$fechacierre,'N');	
		$num= $numerodoc;
		$year=substr($num,11,4);
		$year2=date('Y');
		if($year!=$year2){$num='001-000000';}
		...
		$ObjMov->insertar($_SESSION['IdSucursal'],3,2,$numsiguiente,11,'A',$fechacierre,'D',$montoAperturaDolares/1.19,($montoAperturaDolares/1.19)*(19/100),$montoAperturaDolares,$_SESSION['IdUsuario'],$_SESSION['IdSucursal'],$_SESSION['IdUsuario'],'','',"Cierre de Caja en Dolares del".$fechacierre,'N');
	...
	$idultimomov=$idU->IdMovimiento;
		...
		$ObjMov->insertar($_SESSION['IdSucursal'],3,1,$txtNroDoc,10,'A',$_SESSION['FechaProceso'],'S',$montoAperturaSoles/1.19,($montoAperturaSoles/1.19)*(19/100),$montoAperturaSoles,$_SESSION['IdUsuario'],$_SESSION['IdSucursal'],$_SESSION['IdUsuario'],'','',"Apertura Caja en Soles del ".$_SESSION['FechaProceso'],'N');
		...
		$ObjMov->insertar($_SESSION['IdSucursal'],3,1,$numsiguiente2,10,'A',$_SESSION['FechaProceso'],'D',$montoAperturaDolares/1.19,($montoAperturaDolares/1.19)*(19/100),$montoAperturaDolares,$_SESSION['IdUsuario'],$_SESSION['IdSucursal'],$_SESSION['IdUsuario'],'','',"Apertura Caja en Dolares del ".$_SESSION['FechaProceso'],'N');
	...
	$idultimomov=$idU->IdMovimiento;
		...
		$ObjMov->insertar($_SESSION['IdSucursal'],3,1,$numsiguiente,10,'A',$_SESSION['FechaProceso'],'D',$montoAperturaDolares/1.19,($montoAperturaDolares/1.19)*(19/100),$montoAperturaDolares,$_SESSION['IdUsuario'],$_SESSION['IdSucursal'],$_SESSION['IdUsuario'],'','',"Apertura Caja en Dolares del ".$_SESSION['FechaProceso'],'N');
...
$rst2=$objMov->consultarPedido(2,$idventa,NULL,NULL,NULL,NULL,NULL,NULL,NULL);
...
function referenciar($idventa){
	...
	$rst3=$objMov->consultarPedido(2,$idventa,$idsucursal,NULL,NULL,NULL,NULL,NULL,NULL);
...
require('../negocio/cls_movimiento.php');
	$objMov=new clsMovimiento(); 
	$obj=new xajaxResponse();
	$dato=$objMov->consultarPedido(2,NULL,$idsucursal,$numero,$idtipodoc,NULL,NULL,NULL,NULL);
...
function buscarventasguia($numero,$fechainicio,$formapago){
	...
	$objMovimiento = new clsMovimiento();
	//si consultamos todasd:
	$rst = $objMovimiento->consultarPedido(2,NULL,NULL,$numero,2,$fechainicio,NULL,$formapago,'1');
...
function referenciarguiaremision($idventa){
	...
	$rst3=$objMov->consultarPedido(2,$idventa,NULL,NULL,NULL,NULL,NULL,NULL,NULL);
	$detalle = $rst3->fetchObject();
...
function generarguias($numero,$fechainicio,$fechafin){
	...
	$rst = $objMovimiento->consultarPedido(2,NULL,$idsucursal,$numero,5,$fechainicio,$fechafin,NULL, NULL);
	...
	while($dato = $rst->fetchObject()){
		...
		$registro.="<td align='center'>".$dato->numero."</td>";
	$rst2=$objMovimiento->consultarPedido(2,$dato->idmovimientoref,NULL,NULL,NULL,NULL,NULL,NULL,NULL);
	$dato2=$rst2->fetchObject();
	if($dato2->numero=='0'){
...
function generarguiasinicio(){
	...
	$m=$m-1;
	$m=substr('00',0,2-strlen($m)).$m;
	$rst = $objMovimiento->consultarPedido(2,NULL,$idsucursal,NULL,5,date("Y-".$m."-d"),$fechaproceso,NULL,NULL);
...
function generarpedidossucursal($numero,$idtipodoc,$fechainicio,$fechafin,$formapago, $conguia){
	Global $idsucursal;
	Global $idusuario;
	if($idtipodoc==0){$idtipodoc=NULL;}
	if($formapago=='F'){$formapago=NULL;}
	if($conguia=='0'){$conguia=NULL;}
	$registro.="<table width='1050'  class='registros'>
	<tr>
	<th width='129'>N&deg;</th>
	<th width='100'>FECHA</th>
	<th width='150' title='Sucursal de donde se traeran los productos'>SUCURSAL ORIGEN</th>
	<th width='150' title='Sucursal que solicita los productos'>SUCURSAL DESTINO</th>
	<th width='144'>RESPONSABLE</th>
	<th width='76'>TOTAL</th>
	<th width='70'>MONEDA</th>
	<th width='80'>COMENTARIO</th>
	<th width='76'>ESTADO</th>
	<th colspan='4'>OPERACIONES</th>
	</tr>";
	...
	$rst = $objMovimiento->consultarPedidoSucursal(5,NULL,$idsucursal,$numero,$idtipodoc,$fechainicio,$fechafin,$formapago, $conguia);
	...
	while($dato = $rst->fetchObject()){
	   ...
	   if($cont%2) $estilo="par"; else $estilo="impar";
		...
		$registro.="<tr class='$estilo'>";//<td>".$dato->documento."$m</td>";
		$estado='Normal';
			if(isset($dato->numguia)){
				$estado="Atendido";
			}elseif($dato->idtipodocumento==15){
				$estado="Despachado";
			}else{
				$estado="Pendiente";
			}	
		...
		$registro.="<td align='center'>".$dato->cliente.' '.$dato->acliente."</td>";
		$registro.="<td align='center'>".$dato->sucursal."</td>";
		$registro.="<td align='center'>".$dato->responsable.' '.$dato->aresponsable."</td>";
		$registro.="<td align='center'>".$dato->total."</td>";
		$registro.="<td align='center'>".$dato->moneda."</td>";
		if($dato->comentario==""){
	...
	$registro.="<td align='center'>".$estado."</td>";
	$registro.="<td width='54'><a href='frm_detallepedidosucursal.php?IdVenta=".$dato->idmovimiento."'>Ver Detalle </a></td>";
	
	if($idsucursal==$dato->idcliente){
		if(isset($dato->numguia) or $dato->idtipodocumento==15){
				//$estado="Despachado";
				$registro.="<td></td>";
		}else{
				//$estado="Pendiente";
				$registro.="<td><a href='frm_despachapedidosucursal.php?accion=ACTUALIZAR&IdPedido=".$dato->idmovimiento."'>Despachar</a></td>";
		}	
		
	}else{	$registro.="<td></td>";
	}
	
	if($estado!="Anulado" and $estado!="Despachado" and $estado!="Atendido"){
		if($idsucursal==$dato->idsucursal){
			$registro.="<td><a href='../negocio/cont_venta.php?accion=ANULAR&IdVenta=".$dato->idmovimiento."&IdUsuario=".$idusuario."&IdSucursal=".$idsucursal."'>Anular</a></td>";
		}else{	
			$registro.="<td></td>";
		}
	}else{$registro.="<td></td>";
...
$fcambiaStockPedidoSucursal = & $xajax->registerFunction("cambiaStockPedidoSucursal");
$fcambiaStockPedidoSucursal->setParameter(0,XAJAX_INPUT_VALUE,'txtIdProductoSeleccionado');
$fcambiaStockPedidoSucursal->setParameter(1,XAJAX_INPUT_VALUE,'cboUnidad');
$fcambiaStockPedidoSucursal->setParameter(2,XAJAX_INPUT_VALUE,'cboMoneda');
$fcambiaStockPedidoSucursal->setParameter(3,XAJAX_INPUT_VALUE,'txtIdPersona');

$xajax->registerFunction("genera_cboUnidadPedidoSucursal");

$flistadoPedidoSucursal = & $xajax-> registerFunction('listadoProductoPedidoSucursal');
$flistadoPedidoSucursal->setParameter(0,XAJAX_INPUT_VALUE,'cboCategoria');
$flistadoPedidoSucursal->setParameter(1,XAJAX_INPUT_VALUE,'campo');
$flistadoPedidoSucursal->setParameter(2,XAJAX_INPUT_VALUE,'frase');
$flistadoPedidoSucursal->setParameter(3,XAJAX_INPUT_VALUE,'Pag');
$flistadoPedidoSucursal->setParameter(4,XAJAX_INPUT_VALUE,'TotalReg');
$flistadoPedidoSucursal->setParameter(5,XAJAX_INPUT_VALUE,'cboMoneda');
$flistadoPedidoSucursal->setParameter(6,XAJAX_INPUT_VALUE,'txtIdPersona');
...
$fgenerarpedidossucursal = & $xajax->registerFunction("generarpedidossucursal");
$fgenerarpedidossucursal->setParameter(0,XAJAX_INPUT_VALUE,'txtNumero');
$fgenerarpedidossucursal->setParameter(1,XAJAX_INPUT_VALUE,'cboTipoDoc');
$fgenerarpedidossucursal->setParameter(2,XAJAX_INPUT_VALUE,'txtFechaRegistro');
$fgenerarpedidossucursal->setParameter(3,XAJAX_INPUT_VALUE,'txtFechaRegistro2');
$fgenerarpedidossucursal->setParameter(4,XAJAX_INPUT_VALUE,'cboFormaPago');
$fgenerarpedidossucursal->setParameter(5,XAJAX_INPUT_VALUE,'cboVer');

$fgenerarpedidoventas = & $xajax->registerFunction("generarpedidoventas");
$fgenerarpedidoventas->setParameter(0,XAJAX_INPUT_VALUE,'txtNumero');
$fgenerarpedidoventas->setParameter(1,XAJAX_INPUT_VALUE,'cboTipoDoc');
$fgenerarpedidoventas->setParameter(2,XAJAX_INPUT_VALUE,'txtFechaRegistro');
$fgenerarpedidoventas->setParameter(3,XAJAX_INPUT_VALUE,'txtFechaRegistro2');
$fgenerarpedidoventas->setParameter(4,XAJAX_INPUT_VALUE,'cboFormaPago');
$fgenerarpedidoventas->setParameter(5,XAJAX_INPUT_VALUE,'cboVer');
...
$xajax->registerFunction("generarpedidosventasinicio");
